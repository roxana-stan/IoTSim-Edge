...
public class TestCreateIoTdevice {
	...
	public void create(){
		...
		double transferFrequency = 10d;
		
		int processingAbility = 1;
		IoTDevice device = createIoTDevice(networkModel, id, data_frequency, complexityOfDataPackage, dataSize,
				maxBatteryCapacity, batteryDrainageRateForSensing, processingAbility, capacityToStore, transferFrequency);
	}

	private IoTDevice createIoTDevice(NetworkModel networkModel, int id, int data_frequency,
			int complexityOfDataPackage, int dataSize, int maxBatteryCapacity, int batteryDrainageRateForSensing,
			int processingAbility, long capacityToStore, double transferFrequency) {
		// TODO Auto-generated method stub
		IoTDevice device2 = new TemperatureSensor(networkModel, capacityToStore, transferFrequency);
		return device2;
	}
